...
本仓库是 [Remotely Save](https://github.com/remotely-save/remotely-save) 的社区维护延续分支。

目标是在当前 Obsidian 版本和云服务 API 持续变化的情况下，让 Remotely Save 继续可用，同时尽量兼容已有 Remotely Save 用户。
...
保留相同插件 ID 可以让已有用户继续使用现有插件配置和 OAuth 授权信息：

```text
<你的 Vault>/.obsidian/plugins/remotely-save/data.json
```

不要删除或分享 `data.json`。其中可能包含敏感 token 和凭据。

## 当前维护重点

- 让 OneDrive 同步在近期 Obsidian 桌面端和移动端版本中继续工作。
- Review 并吸收社区 fork 和原仓库 PR 中有价值的修复。
- 尽量保持发布保守，兼容现有 Vault。
- 发布可通过 BRAT 或手动安装的 GitHub Release 产物。

## OneDrive HTTP 400 修复

版本 `0.5.25.1` 包含以下 OneDrive 请求层修复：

https://github.com/Deadbush225/remotely-save/commit/eaea5f118e44faa04ae9ed1e6e81da1792cb4df0

该修复将以下文件中的 Obsidian `request()` 替换为 `requestUrl()`：
...
- OAuth 授权码换 token
- refresh token 刷新
- Microsoft Graph JSON GET/POST/PATCH 请求

## 使用 BRAT 安装本分支

本分支可以通过 Obsidian 插件 [Obsidian42 - BRAT](https://github.com/TfTHacker/obsidian42-brat) 安装和更新。BRAT 可以直接从 GitHub release 安装插件，适合在插件还没有进入官方 Obsidian 社区插件市场前使用社区维护版。

步骤：

1. 在 Obsidian 中安装并启用 [Obsidian42 - BRAT](https://github.com/TfTHacker/obsidian42-brat)。
2. 打开 BRAT 设置。
...
5. 让 BRAT 从最新 GitHub release 安装插件。
6. 重启 Obsidian。
7. 确认 Remotely Save 已启用。
8. 在已备份的 Vault 中测试同步。

该仓库名用于通过 BRAT 安装本维护分支。
...
```text
<你的 Vault>/.obsidian/plugins/remotely-save/
```
...
本仓库不是官方上游仓库，而是社区维护的延续分支。

原项目：

https://github.com/remotely-save/remotely-save

原仓库中的有用 PR、issue 和社区修复可能会被 review 并移植到这里。
